<?php
/**
 * Shows a login form and authenticates the Admin member.
 *
 * If the Admin is not logged in it will show a login form. If login data is
 * posted it will check that data against the database and authenticate the
 * Admin member if the login data is correct.
...
$page_type = 'special';
$Member = new Member('admin', $settings);        // Create Member object to check logins etc
...
if (FALSE != $Member->isLoggedIn($reason)) {
    header('Location: '.$settings['admin_uri'].'/');
    exit;
}
...
$output .= $Form->startFieldset('Admin Panel Log In');
$info  = 'Please login to the Admin Panel with your login name or member ';
$info .= 'number and your account password.';
$output .= $Form->showInfo($info);
$output .= $Form->showTextInput('Login Name or Member Number', 'login_id', 255, 0);
